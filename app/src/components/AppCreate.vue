  ...
  <div id="app-create">
    <form @submit.prevent="save">
      <div class="form">
        ...
        <div class="line">
          <label for="app">Tipo de aplicação:</label>
          <div class="items">
            <div class="item">
        ...
        <div class="line">
          ...
          <input type="text" id="domain" v-model="form.path" required />
          <span class="result">
            <span>http://{{ domain }}/{{ form.path }}</span> |
            <span>http://{{ form.path }}.{{ domain }}</span>
          </span>
          <span v-if="error?.errors?.path" class="error">
            {{ error.errors.path.join(",") }}
          </span>
        </div>
        <div class="line">
...
async function save() {
  try {
    ...
    const { data } = await http.post<App>("apps", request);
    store.create(data);
    emit("close");
  } catch ({ response }: any) {
