...
const appDocumentTypes = ref<AppDocumentType[]>([])
...
onMounted(async () => {
  ...
  const { data: users } = await http.get<AppUser[]>(
    `api/app/${app.value.id}/users`
  )
  const { data: document_types } = await http.get<AppDocumentType[]>(
    `api/app/${app.value.id}/document_types`
  )

  appUsers.value = users
  appDocumentTypes.value = document_types
})
</script>
