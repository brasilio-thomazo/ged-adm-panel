...
class AppController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $base = sprintf("/%s/resources/kubernetes", $this->home);
    ...
    private function makeSecret(Request $request, App $app, array $names, array &$data)
    {
        ...
        $url = URL::signedRoute('app.subscribe', ['app' => $app->id]);
        ...
        $yaml['data']['SUBSCRIBE_URL'] = base64_encode($url);
    ...
    public function store(StoreAppRequest $request)
    {
        ...
        $this->makeSecret($request, $app, $names, $data);
    ...
    public function subscribe(App $app, Request $request)
    {
        $update = [];
        if (!$app->installed_at) {
    ...
    public function userList(App $app)
    {
        $url = sprintf("http://localhost:8010/user", $app->path);
        $response = Http::get($url);
        return response($response->json());
    }

    public function documentTypeList(App $app)
    {
        $token = Cache::get("token_" . $app->id);
        if (!$token) {
            $reply = Http::post("http://localhost:8010/api/login", ["username" => "system", "password" => "system", "device_name" => "adm-panel"]);
            Cache::put("token_" . $app->id, "Bearer " . $reply->json("token"));
        }
        $reply = Http::withHeaders(['Authorization' => $token])->get("http://localhost:8010/api/document_type");
        if ($reply->successful()) {
